    ...
    <div class="admin-wrapper flex flex-row">
        ...
        <div class="admin-content p-5 flex flex-row flex-wrap justify-center items-center w-[85%] rounded-tl-[2rem] rounded-bl-[2rem] bg-gray-200">
            ...
            <div class="title-pagination w-full p-3 flex justify-between">
                <h1 class="text-3xl">Manga news</h1>
                {{$mangadata->links()}}
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <!-- Dynamically generated Anime Cards -->
                @foreach($mangadata as $anime)
                <div class="anime-card p-4 bg-white">
                    <h2 class="text-lg font-semibold mb-2">{{ $anime->Manga_title }}</h2>
                    <img src="banner_images/manga_banner_images/{{ $anime->image_url }}" alt="{{ $anime->Manga_title }}" class="mb-4">
                    
                    <div class="flex justify-between mt-4">
                        <a href="" class="text-blue-500 hover:text-blue-700">Update</a>
                        <a href="{{route('MangablogPage.dataFetch',$anime->id)}}" class="text-blue-500 hover:text-blue-700">View</a>
                        <a href="{{route('admin.databse.mangaTableDelete',$anime->id)}}" class="text-red-500 hover:text-red-700">Delete</a>
                    </div>
                </div>
            @endforeach
